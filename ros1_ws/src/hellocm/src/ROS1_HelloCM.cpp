...
static struct {
	...
	struct {
	    tRosIF_TpcSub<hellocm_msgs::Command>  Command;
	} Sub2;
	
	/* testing rosbag/
	struct {
	    tRosIF_TpcSub<hellocm_msgs::cntrl_cmd>  cntrl_cmd;
	} Sub3;
    */
...
static double lateral_error;
static double DeltaAng;
static double DevAngToRoute;
static double DevDistToRoute;
static double velocity;
static int vz;
static double steering_angle, steering;
static double brake;
static double gas;
...
ros_HelloCM_CB_TpcIn(const hellocm_msgs::CM2Ext::ConstPtr &msg)
{
    ...
    auto in = &RosIF.Topics.Sub.CM2Ext;
    RosIF.SynthDelay = msg->synthdelay;
    ...
    LOG("%s - TpcIn (ROS Time=%.3fs): CarMaker Node is in cycle %lu, Time=%.3fs, Velocity=%.3fm/s, Steering Angle = %.3f, delta angle = %.3f,  Gas = %.3f, Brake = %.3f,  DevAngToRoute = %.3f,  deviation =%.3fm, PositionX = %.3f, PositionY = %.3f, VPositionX = %.3f, VPositionY = %.3f, LastPositionX = %.3f, LastPositionY = %.3f, Stamp=%.3fs, SeqID=%d, vz=%d",
...
static void
ros_Mapnode_CB_TpcIn(const hellocm_msgs::Command::ConstPtr &msg)
{
	steering = msg -> steering;
	
	LOG("Steering Angle = %.3f",
	    msg->steering);
}

/* testing rosbag/
static void
ros_Mapnode_CB_TpcIn(const hellocm_msgs::cntrl_cmd::ConstPtr &msg)
{
	steering = msg -> steering;
	gas = msg->throttle;
	LOG("Steering Angle = %.3f, throttle = %.3f",
...
ros_HelloCM_CB_PubTimer(const ros::TimerEvent& Event) {
    ...
    //steering_angle =  (1 * DeltaAng + std::atan(20  * vz * DevDistToRoute/ (velocity + 0.1)))*3;
    //double lateral_error = DevDistToRoute * std::tan(DevAngToRoute);
    ...
    if(-DevDistToRoute > 20){
		...
		steering_angle =  -1  * 2.0;
		//gas = 0.2;
	}
	else{
		steering_angle =  (1 * DeltaAng + std::atan(10  * vz * DevDistToRoute/ (velocity + 0.1)))/3;//Stanley Controller
		//gas= 1.0;
	}
	
	
	
    //double steering_angle = std::atan(2.75 * 2 *( std::sin(DevAngToRoute) )/ (DevDistToRoute/std::sin(DevAngToRoute)))/3;
    if (-DevDistToRoute < 8.0 && velocity > 5.0)
		brake = 1.0;
	else if(-DevDistToRoute > 22.0 && velocity > 5.0)
		brake = 1.0;
	else
		brake = 0.0;
    ...
    out->Msg.Brake = brake;
    out->Msg.SteeringAng = steering;
    //out->Msg.SteeringAng = steering_angle;
    out->Msg.Gas = gas;
   ...
   //LOG("%s - PubTimer (ROS Time=%.3fs): This Node is in cycle %lu, Time=%.3fs, Brake =%.3f, Steering angle = %.3f,  lateral error =%.3fm, Stamp=%.3fs",
	 //   ros::this_node::getName().c_str(), ros::Time::now().toSec(),
	  //  out->Msg.cycleno, out->Msg.time.toSec(), out->Msg.Brake,  out->Msg.SteeringAng, DevDistToRoute, out->Msg.header.stamp.toSec());
}
...
ros_HelloCM_Init(int Argc, char **Argv) {
    ...
    /* Subscribe from ROS2 specific */
    LOG("  -> Subscribe '%s'", hellocm::tpc_in_name2.c_str());
    RosIF.Topics.Sub2.Command.Sub = node->subscribe(hellocm::tpc_in_name2, 10, ros_Mapnode_CB_TpcIn);
    
     /* rosbag testing/
    LOG("  -> Subscribe '%s'", hellocm::tpc_in_name3.c_str());
    RosIF.Topics.Sub3.cntrl_cmd.Sub = node->subscribe(hellocm::tpc_in_name3, 10, ros_Mapnode_CB_TpcIn);
    */
